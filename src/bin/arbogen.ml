...
let speclist =
  ...
  [("-version", Arg.Unit (fun () -> Format.printf "%s@." version_str; exit 0),
   ...
   ("-methrec", Arg.Int (function
        | x -> global_options.int_methrec <- x; global_options.methrec <- true),
	"<n> set the size of object generated by the recursive method\n");

   ("-bp", Arg.Int (function
        | (-1) -> global_options.bp <- -1
	| 0 -> global_options.bp <- 0
	| _ ->
          Format.eprintf "Error: bp must be in 0 for boustrophedonic order or (-1) for lexicographic";
          exit 1),
	"<n> set the order for recursive method\n")
...
let () =
  ...
  let result, wgrm = match state with
    | None ->
      ...
      if global_options.methrec then begin
	let n = global_options.int_methrec in
	print_string "Start counting (may be long if n > 5000)...\n";
        ...
        for j = 0 to ((Array.length countArrays) -1) do
          ...
          print_string "[ ";
	  let jj = (if n < 15 then n else 15) in
          for ww = 0 to jj do
            Printf.printf "%s " (Z.to_string (Array.get (Array.get countArrays j) ww));
          done;
          print_string " ]\n";
        done; 
	Printf.printf "\nGenerating Tree of size %d with recursive method...\n" n;
	let tree = RecursiveMethod.generator specs countArrays 0 n global_options.bp in
     	print_tree tree;
	print_string "\n"
      end;
